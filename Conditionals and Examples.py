...
'''
Conditionals change the outcome of a program's execution run, depending on \
what the program is doing at the time. The conditionals in Python are the 'if:' \
statement, 'elif:' statement and the 'else:' statement, along with the 'True:' and \
'False:' statements. Conditionals are mainly used in conjunction with 'input' statements \
and conditional while-loops. However, Logical operators are also used to test whether \
a condition is less than (<), greater than (>), less than (<=) or equal to, greater than (>=) \
or equal to, equals (==) equals and not (!=) equal to. For example, 5 is greater (>) than 4 \
and 4 is less (<) than 5. Here are a few examples of logical operators, which test integer \
values against other integer values within 'print' statements. These 'print' statement \
illustration examples below will either display on the screen output as "True" or "False", \
...
is simply all about the concept of decision making laws, meaning if something is true, then \
it is not false. Likewise, if something is false, then it is not true.

When it comes to program development, sometimes logical operators aren't enough alone to \
do the job. In most cases, other conditionals are needed to help the logical operators do the \
job. With the 'if:', 'elif:', 'else', 'true', 'false', 'and', 'or' and 'not' conditionals, the logical operators \
can do the job as they were designed for doing, which are to test values against other values and \
comparing data against user input data.
'''
